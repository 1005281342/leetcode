...
class Solution:
    def minRemoveToMakeValid(self, s: str) -> str:
        ...
        lq = len(tq)
        if lp > lq:
            for _ in range(lp - lq):
                i = tp.pop()
                ts[i] = ''
        elif lp < lq:
            for _ in range(lq - lp):
                i = tq.pop()
                ts[i] = ''
        return ''.join(ts)
